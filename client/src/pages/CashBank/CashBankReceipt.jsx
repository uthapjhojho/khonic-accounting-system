...
const CashBankReceipt = () => {
    ...
    const fetchInitialData = async () => {
        ...
        try {
            const [voucherData, rawAccounts] = await Promise.all([
                journalService.getVoucherNumber('KTMC'),
    ...
    const isHeaderValid = () => {
        ...
        if (!memo) {
            alert('Isi memo transaksi.');
            return false;
        }
        if (totalAmount <= 0) {
            alert('Jumlah total transaksi harus lebih dari 0.');
            return false;
        }
        return true;
    };
    ...
    const isLinesValid = () => {
        if (lines.length === 0) {
            alert('Tambahkan setidaknya satu baris transaksi.');
            return false;
        }
        for (const line of lines) {
            if (!line.account) {
                alert('Pilih akun untuk setiap baris transaksi.');
                return false;
            }
            if (parseRupiah(line.amount) <= 0) {
                alert('Isi jumlah yang valid (lebih dari 0) untuk setiap baris transaksi.');
                return false;
            }
        }
        return true;
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await cashBankService.saveReceipt({
    ...
    return (
        <Layout>
            <div className="max-w-5xl mx-auto pb-20">
                <PageHeader
                    title={nav.receipt.label}
                    breadcrumbs={[
                        { label: nav.receipt.label, path: nav.receipt.path },
                        { label: 'Tambah Baru' }
                    ]}
                />
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-8 relative min-h-[400px]">
                    ...
                    <div className="flex justify-end gap-3 mt-10">
                        ...
                        <button
                            ...
                            onClick={handleSave}
                            className={`px-10 py-3 rounded-xl font-bold transition-all text-sm ${isSaveDisabled ? 'bg-gray-100 text-gray-300 cursor-not-allowed shadow-none' : 'bg-green-600 text-white hover:bg-green-700 shadow-lg shadow-green-100'}`}
                        >
