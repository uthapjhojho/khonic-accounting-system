    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let loadedLines = journal.lines || [];
                while (loadedLines.length < 2) {
                    loadedLines.push({ id: Date.now() + loadedLines.length, account: '', debit: 0, credit: 0 });
                }
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                type="text"
                                className="w-full px-4 py-2.5 rounded-lg border border-gray-100 bg-gray-50 text-gray-500 outline-none cursor-not-allowed"
                                value={journalNumber}
                                disabled
                            />
                        </div>
